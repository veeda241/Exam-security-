...
from schemas.event import ML_EVENT_TYPES, EventCreate, EventIngestResponse, EventResponse
from supabase_client import get_supabase
from workers.utils import (
    get_active_risk_config,
    insert_event,
    publish_session_message,
    recompute_and_update_session_risk,
)
...
def _enqueue_ml_job(event_type: str, session_id: str, payload: dict) -> str | None:
    ...
    if event_type == "frame_sample" and frame_b64:
        if _get_session_monitoring_tier(session_id) == "reduced":
            return None

        from workers.face_worker import process_frame as face_task
        from workers.gaze_worker import process_frame as gaze_task
        from workers.object_worker import process_frame as object_task
    ...
    if event_type == "screenshot" and frame_b64:
        from workers.ocr_worker import process_frame as ocr_task
        job = ocr_task.delay(session_id, frame_b64)
        return job.id

    if event_type == "answer_submit" and text:
        from workers.nlp_worker import check_similarity
...
async def ingest_event(event_in: EventCreate, user=Depends(get_current_user), db=Depends(get_db)):
    if not db:
        raise HTTPException(status_code=503, detail="Database unavailable")

    if not get_rate_limiter().allow(event_in.session_id):
    ...
    if session_res.data[0].get("status") != "active":
    ...
    event = insert_event(event_in.session_id, normalized_type, event_in.payload, weight=weight)
    if event:
    ...
    return EventIngestResponse(status="accepted", event_id=event.get("id") if event else None)


@router.get("/session/{session_id}", response_model=List[EventResponse])
async def list_session_events(session_id: str, user=Depends(get_current_user), db=Depends(get_db)):
    if not db:
        raise HTTPException(status_code=503, detail="Database unavailable")
