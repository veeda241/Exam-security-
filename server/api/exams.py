...
from deps import get_current_user, get_db, get_proctor_user
...
async def create_exam(exam_in: ExamCreate, user=Depends(get_proctor_user), db=Depends(get_db)):
    if not db:
...
async def list_exams(user=Depends(get_current_user), db=Depends(get_db)):
    if not db:
        return []
    try:
...
async def get_exam(exam_id: str, user=Depends(get_current_user), db=Depends(get_db)):
    if not db:
        raise HTTPException(status_code=503, detail="Database unavailable")
...
async def update_exam(exam_id: str, exam_in: ExamUpdate, user=Depends(get_proctor_user), db=Depends(get_db)):
    if not db:
        raise HTTPException(status_code=503, detail="Database unavailable")
    ...
    if "ruleset" in updates and updates["ruleset"] is not None:
        updates["ruleset"] = (
            updates["ruleset"].model_dump()
            if hasattr(updates["ruleset"], "model_dump")
            else updates["ruleset"]
        )
