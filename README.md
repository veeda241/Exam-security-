...
│   │   └── weights/         # Downloaded model artifacts (ignored by Git)
│   ├── reports/templates/   # Jinja2 report templates used by report workers
...
# Configure server/.env (copy from server/.env.example) with the new
# SUPABASE_URL, SUPABASE_SECRET_KEY, SUPABASE_JWKS_URL, and PG_* pooler
# credentials. Keep SUPABASE_SECRET_KEY server-side only.
...
```
Model weights belong in `server/services/weights/` and are intentionally not
committed. The MediaPipe face landmarker bundle remains at
`server/services/face_landmarker.task` for compatibility.
...
The V2 API exposes authenticated telemetry upload routes at
`/api/v1/uploads/screenshot`, `/api/v1/uploads/webcam`, and
`/api/v1/uploads/latest/{session_id}?type=screenshot|webcam`. Uploaded frames
are stored under `server/uploads/` locally and broadcast over the session
WebSocket; production deployments should use object storage for durable
retention.
...
4. Configure environment variables (secrets should be set in the Render dashboard, not committed):
   ...
   - `SUPABASE_PUBLISHABLE_KEY` only where a browser-side Supabase client is used
